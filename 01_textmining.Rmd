...
We'll look at some books by [Jane Austen](https://en.wikipedia.org/wiki/Jane_Austen), an 18th century novelist explored women and marriage within the British upper class.  Austen has a unique and well earned following within literature. Her works is consistently discussed and honored. To this day, Austen's novels are the source of many adaptations, written and on-screen.  Through the `janeaustenr` package we can access and mine the text of six Austen novels.  We can call the collection of novels a corpra.  An individual novel is a corpus.
...
The default tokenizing moded is "words". With the `unnest_tokens()` function, tokens can be:  **words**, characters, character_shingles, **ngrams**, skip_ngrams, **sentences**, lines, paragraphs, regex, tweets, and ptb (Penn Treebank). 
...
> **Algorithim:** sentiment = positive - negative
...
```{r emma afinn}
emma_afinn_sentiment %>% 
  mutate(feeling = cut_interval(sentiment, n = 5)) %>% 
  ggplot(aes(index, sentiment)) +
